...
// the output of the encoding process for predictive coding is
// first (N) values are signal
// next (signal length - N) values are differences
...
void reconstructFromDifferentialEncoding(float [] s, float [] p, float [] e) {
  int N = DIFFERENTIAL_ENCODING_SIZE;

  // depending on the number of terms in the linear predictor 
  // the signal is the predicted value with no error
  for (int i = 0; i < N; i += 1) {
    s[i] = p[i];
  }
